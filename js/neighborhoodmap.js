    ...
    //getting the variables stored in sessionStorage
      ...
      function mainFunc() {
        ...
        arrayUtils, parser, Query, QueryTask, HomeButton, Print, dom, Button, esriConfig, SpatialReference) {
              ...
              //neighborhoods.setLabelingInfo([ labelClass ]);
                  ...
                  for (var j = 1, len = $scope.classBreaks.classBreakInfos.length; j < len; j++){
                      ...
                      if(resultItems[i] >= $scope.classBreaks.classBreakInfos[j-1].classMaxValue && resultItems[i] <= $scope.classBreaks.classBreakInfos[j].classMaxValue){
                          symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,new SimpleLineSymbol(
                            ...
                            new Color($scope.classBreaks.classBreakInfos[j].symbol.outline.color), 1
                            ), new Color($scope.classBreaks.classBreakInfos[j].symbol.color));
